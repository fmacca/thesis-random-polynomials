...
folder_name='Results/Degree02_NoiseCircular_SimulTrajectoriesDoubleRoot'; %Name for the results folder: it should be named after the kind of test performed
...
K=6; % Number of iterations per simulation (n of noisy measurements per polynomial)
...
for k=1:K
    for t=1:T
        plot(real(r_n(1,k,t)),imag(r_n(1,k,t)),col(1+mod(k,size(col,1)),:),'MarkerSize',1); hold on; % Simulated roots
        plot(real(r_n(2,k,t)),imag(r_n(2,k,t)),col(1+mod(k,size(col,1)),:),'MarkerSize',1); hold on;
    end
end
