...
folder_name='Results/Degree02_NoiseCircular_SimulMseVsSnr_DiscCond'; %Name for the results folder: it should be named after the kind of test performed
...
r=[1; -1];%[scale*(2*rand(N,1)-1)+scale*1i*(2*rand(N,1)-1)];
% Compute corresponding noise-free polynomial cefficients
a=conj(poly(r)');
% Compute discriminant and conditional number of discriminant matrix
[discr,cnd] = discriminant_cond(a);
...
for ii=1:SNR_nsteps
    sigma_a=(sqrt(1/(abs(cnd)*SNRlin(ii))));%sigma_a=(sqrt(abs(discr)/SNRlin(ii)));
...
sgtitle(strcat("Coefficients and roots distibutions vs SNR, discr = ",num2str(discr)));
...
sgtitle(strcat("MSE matrix (real composite representation) vs SNR, discr = ",num2str(discr)));
...
legend(leg1);
title(strcat("MSE error vs SNR, discr = ",num2str(discr)));grid on;
