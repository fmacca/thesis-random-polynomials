...
folder_name='Results/Degree03_NoiseCircular_SimulManova'; %Name for the results folder: it should be named after the kind of test performed
...
%         HotT2_p(ii)=T2Hot1_mod([real(r_n(1,:,ii))'-real(r(1)) imag(r_n(1,:,ii))'-imag(r(1)) real(r_n(2,:,ii))'-real(r(2)) imag(r_n(2,:,ii))'-imag(r(2))]);
        ...
        MBox_p(ii)=MBoxtest_mod([[ones(K,1) real(r_n(1,:,ii))' imag(r_n(1,:,ii))' real(r_n(2,:,ii))' imag(r_n(2,:,ii))' real(r_n(3,:,ii))' imag(r_n(3,:,ii))'];[2*ones(K,1) real(r_n_sim_analytic(1,:,ii))' imag(r_n_sim_analytic(1,:,ii))' real(r_n_sim_analytic(2,:,ii))' imag(r_n_sim_analytic(2,:,ii))' real(r_n_sim_analytic(3,:,ii))' imag(r_n_sim_analytic(3,:,ii))']]);
        
        [Manova_d(ii),Manova_p(ii)]=manova1([[real(r_n(1,:,ii))' imag(r_n(1,:,ii))' real(r_n(2,:,ii))' imag(r_n(2,:,ii))' real(r_n(3,:,ii))' imag(r_n(3,:,ii))'];[real(r_n_sim_analytic(1,:,ii))' imag(r_n_sim_analytic(1,:,ii))' real(r_n_sim_analytic(2,:,ii))' imag(r_n_sim_analytic(2,:,ii))' real(r_n_sim_analytic(3,:,ii))' imag(r_n_sim_analytic(3,:,ii))']],[ones(K,1);2*ones(K,1)]);
    end
...
%     ratio=abs(r(1)-r(2))./abs(Projection);
    ...
    dataset=[dataset; [counter*ones(SNR_nsteps,1) r(1)*ones(SNR_nsteps,1) r(2)*ones(SNR_nsteps,1) r(3)*ones(SNR_nsteps,1) gamma Gauss_test_HZ MBox_p Manova_d Manova_p]];
